...
class ApprovedMission(db.Model):
    child = db.Column(db.Integer, db.ForeignKey("child_user.id", ondelete='CASCADE'), nullable=False, primary_key=True)
    mission = db.Column(db.Integer, db.ForeignKey("mission.id", ondelete='CASCADE'), nullable=False, primary_key=True)